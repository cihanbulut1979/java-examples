...
public class MPAKafkaConsumer {
	public void execute(int partitionNo) {
		...
		TopicPartition partition = new TopicPartition(KafkaConstants.TOPIC_NAME, partitionNo);
						...
						for (Object rec : records) {
							...
							System.out
									.println(String.format("Consumer : " + " Group - %s " + " Topic - %s, Partition - %d,  Offset = %d, Key - %s Value: %s",
											 KafkaConstants.GROUP_ID_CONFIG ,  record.topic(), record.partition(), record.offset(), record.key(), record.value()));
